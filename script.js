const prayerTimes = [
    { name: "FAJR", time: "4:37AM" },
    { name: "DHUHR", time: "12:06PM" },
    { name: "ASR", time: "4:29PM" },
    { name: "MAGHRIB", time: "6:17PM" },
    { name: "ESHA", time: "7:31PM" },
];
...
function updateRamadan() {
    const ramadan1 = new Date(2024, 2, 12, 4, 37, 0, 0);
    const now = Date.now();
...
function getPrayerTimes() {
    ...
    const fajar = new Date(year, month, date, 4, 37, 0, 0);
    const dhuhr = new Date(year, month, date, 12, 6, 0, 0);
    const asr = new Date(year, month, date, 12 + 4, 29, 0, 0);
    const magrib = new Date(year, month, date, 12 + 6, 17, 0, 0);
    const esha = new Date(year, month, date, 12 + 7, 31, 0, 0);
...
function updatePrayers() {
    ...
    if (curTime >= prayerTimes1[prayerTimes1.length - 1]) {
        markPrayer(prayerTimes1.length - 1);
    } else {
        let i = 0;
